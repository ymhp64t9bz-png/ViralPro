...
# Importar corretor LanguageTool
try:
    from .languagetool_corrector import corrigir_titulo_completo
    LANGUAGETOOL_AVAILABLE = True
    logger.info("[LANGUAGETOOL] Corretor ortográfico carregado")
except ImportError:
    LANGUAGETOOL_AVAILABLE = False
    logger.warning("[LANGUAGETOOL] Corretor não disponível (instale: pip install language-tool-python)")
...
MODEL_PATH_LLAMA = get_model_path('Qwen2.5-7B-Instruct-Q4_K_M.gguf')
...
def load_llama_model():
    ...
    if _LLAMA_INSTANCE is None:
        if not os.path.exists(MODEL_PATH_LLAMA):
            logger.error(f"[QWEN] Modelo não encontrado em: {MODEL_PATH_LLAMA}")
            return
        
        logger.info("[QWEN] Carregando Qwen 2.5 7B Instruct...")
        try:
            ...
            logger.info("[QWEN] Modelo carregado (CPU).")
        except ImportError:
            logger.error("[QWEN] Biblioteca 'llama-cpp-python' não instalada.")
            _LLAMA_INSTANCE = None
        except Exception as e:
            logger.error(f"[QWEN] Erro ao carregar: {e}")
            _LLAMA_INSTANCE = None

def unload_llama_model():
    global _LLAMA_INSTANCE
    if _LLAMA_INSTANCE is not None:
        logger.info("[QWEN] Descarregando modelo...")
        del _LLAMA_INSTANCE
        _LLAMA_INSTANCE = None
        _clean_memory()
...
def generate_viral_title_local(anime_name, dialogue):
    ...
    try:
        ...
        logger.error(f"[QWEN ANALYZER] Erro título: {e}")
        return f"{anime_name} - CENA DE AÇÃO"
    finally:
        unload_llama_model()

# ==================== LIMPEZA DE RESPOSTA QWEN 2.5 ====================

def clean_llama_response(raw_text):
    """
    Limpeza simples para Qwen 2.5 (resposta direta e limpa).
...
def generate_viral_title_batch(anime_name, dialogue):
    ...
    try:
        ...
        # Prompt otimizado para Qwen 2.5 (português perfeito)
        system_prompt = (
            "Você é um gerador de títulos virais para TikTok. "
            "Crie um título em PORTUGUÊS BRASILEIRO, com ORTOGRAFIA PERFEITA. "
            "Responda APENAS com o título, sem explicações. "
            "Use 4-9 palavras, TUDO EM MAIÚSCULAS. "
            "IMPORTANTE: Escreva cada palavra CORRETAMENTE, sem erros ortográficos."
        )
        ...
        logger.info(f"[QWEN] Gerando título para {anime_name}...")
        ...
        logger.info(f"[QWEN] Resposta crua: {raw_response[:100]}...")
        
        # Limpa resposta (simples para Qwen 2.5)
        titulo = clean_llama_response(raw_response)
        
        # Se após limpeza estiver vazio, usa fallback baseado no diálogo
        if not titulo or len(titulo) < 10:
            logger.warning("[QWEN] Título vazio após limpeza. Usando fallback.")
        ...
        if LANGUAGETOOL_AVAILABLE:
            logger.info("[LANGUAGETOOL] Corrigindo ortografia...")
            titulo_antes = titulo
            titulo = corrigir_titulo_completo(titulo).upper()
            
            if titulo != titulo_antes:
                logger.info(f"[LANGUAGETOOL] ✅ Corrigido!")
                logger.info(f"[LANGUAGETOOL] Antes: {titulo_antes}")
                logger.info(f"[LANGUAGETOOL] Depois: {titulo}")
            else:
                logger.info("[LANGUAGETOOL] ✅ Título já estava correto")
        else:
            logger.error("[LANGUAGETOOL] ❌ CORRETOR NÃO DISPONÍVEL - ERRO CRÍTICO!")
        # ==================== FIM DA CORREÇÃO ====================
        ...
        logger.info(f"[QWEN] Título final: {titulo}")
        return titulo
            
    except Exception as e:
        logger.error(f"[QWEN] Erro ao gerar título: {e}")
        import traceback
        logger.error(f"[QWEN] Traceback: {traceback.format_exc()}")
...
def manually_unload_llama():
    """Descarrega manualmente o modelo Qwen 2.5 após processamento em lote."""
    unload_llama_model()
...
def analyze_viral_segments_deepseek(full_transcript_text, duration_total):
    ...
    SEM LIMITAÇÕES - Permite que o Qwen 2.5 trabalhe até o final.
    """
    try:
        load_llama_model()
        if _LLAMA_INSTANCE is None:
            logger.error("[QWEN ANALYZER] Modelo não carregado para análise de segmentos.")
            return []

        logger.info("[QWEN ANALYZER] Analisando roteiro para cortes automáticos...")
        logger.info("[QWEN ANALYZER] Modo SEM LIMITAÇÕES ativado - Qwen 2.5 trabalhará até concluir")
        ...
        logger.info(f"[QWEN ANALYZER] Enviando {len(full_transcript_text)} caracteres de transcrição...")
        ...
        if len(full_transcript_text) > max_chars:
            logger.warning(f"[QWEN ANALYZER] Transcrição muito longa ({len(full_transcript_text)} chars). Limitando a {max_chars} chars.")
        ...
        logger.info(f"[QWEN ANALYZER] Processando {len(transcript_to_send)} caracteres...")
        logger.info(f"[QWEN ANALYZER] Aguarde - Qwen 2.5 analisando (pode levar 30-60s)...")
        logger.info(f"[QWEN ANALYZER] Iniciando processamento em tempo real...")
        ...
        try:
            logger.info(f"[QWEN ANALYZER] ⏳ Gerando resposta (streaming ativado)...")
                ...
                if 'choices' in chunk and len(chunk['choices']) > 0:
                    ...
                    if 'content' in delta:
                        ...
                        if token_count % 10 == 0:
                            ...
                            logger.info(f"[QWEN ANALYZER] 📝 {token_count} tokens gerados ({elapsed:.1f}s)...")
            
            elapsed_total = time.time() - start_time
            logger.info(f"[QWEN ANALYZER] ✅ Resposta completa! {token_count} tokens em {elapsed_total:.1f}s")
            
            response = response_text.strip()
            
        except Exception as stream_error:
            # Fallback para modo não-streaming se falhar
            logger.warning(f"[QWEN ANALYZER] Streaming falhou: {stream_error}. Usando modo padrão...")
            ...
            elapsed_total = time.time() - start_time
            logger.info(f"[QWEN ANALYZER] Resposta recebida em {elapsed_total:.1f}s")
        
        logger.info(f"[QWEN ANALYZER] Resposta completa recebida ({len(response)} caracteres)")
        logger.info(f"[QWEN ANALYZER] Primeiros 500 caracteres: {response[:500]}...")
        ...
        if not matches:
            logger.warning("[QWEN ANALYZER] Nenhum timestamp no formato [X-Y] encontrado. Tentando padrão alternativo...")
        ...
        if matches:
            logger.info(f"[QWEN ANALYZER] {len(matches)} timestamps encontrados")
            ...
            for start, end in matches:
                ...
                if duration >= 60 and duration <= 180:
                    if s < duration_total:
                        ...
                        logger.info(f"[QWEN ANALYZER] ✓ Segmento válido: {s}s-{e}s (duração: {duration}s)")
                else:
                    logger.warning(f"[QWEN ANALYZER] ✗ Segmento ignorado (duração {duration}s fora do range 60-180s): {s}-{e}")
        else:
            logger.warning("[QWEN ANALYZER] Nenhum timestamp válido encontrado na resposta")
        
        # Fallback automático se necessário
        if not segments and duration_total > 60:
            logger.info("[QWEN ANALYZER] Criando segmentos automáticos como fallback...")
            ...
            for i in range(num_segments):
                ...
                if end - start >= 60:
                    ...
                    logger.info(f"[QWEN ANALYZER] ✓ Segmento automático: {start}s-{end}s")
        ...
        if len(unique_segments) > 5:
            logger.warning(f"[QWEN ANALYZER] {len(unique_segments)} segmentos encontrados. Limitando a 5.")
            unique_segments = unique_segments[:5]
        
        logger.info(f"[QWEN ANALYZER] Total de {len(unique_segments)} segmentos identificados")
        return unique_segments
    except Exception as e:
        logger.error(f"[QWEN ANALYZER] Erro na análise de segmentos: {e}")
        import traceback
        logger.error(f"[QWEN ANALYZER] Traceback: {traceback.format_exc()}")
        return []
